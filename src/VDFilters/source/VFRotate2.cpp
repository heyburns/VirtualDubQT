...
static uint32 __declspec(naked) cc_MMX(const uint32 *src, const int *table) {
	...
	static const sint64 x0000200000002000 = 0x0000200000002000LL;
...
#ifndef _M_AMD64
	if (MMX_enabled) {
#if defined(VD_CPU_X86) && defined(_MSC_VER)
		__asm emms
#endif
	}
#endif
...
static int rotate2_start(VDXFilterActivation *fa, const VDXFilterFunctions *ff) {
	...
	xaccum_base = xaccum_low = -0x80000000LL*fa->src.w;
	yaccum_base = yaccum_low = -0x80000000LL*fa->src.h;
	xaccum_high = 0x80000000LL*fa->src.w;
	yaccum_high = 0x80000000LL*fa->src.h;
