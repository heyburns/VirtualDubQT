...
bool VDDialogFrameW32::Create(VDGUIHandle parent) {
	if (!mhdlg) {
		...
		CreateDialogParamW(g_hInst, MAKEINTRESOURCEW(mpDialogResourceID), (HWND)parent, (DLGPROC)StaticDlgProc, (LPARAM)this);
	}
...
sintptr VDDialogFrameW32::ShowDialog(VDGUIHandle parent) {
	mbIsModal = true;
	return DialogBoxParamW(g_hInst, MAKEINTRESOURCEW(mpDialogResourceID), (HWND)parent, (DLGPROC)StaticDlgProc, (LPARAM)this);
}
