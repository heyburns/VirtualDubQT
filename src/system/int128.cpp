...
#elif !defined(VD_CPU_AMD64) || defined(__GNUC__) || defined(__clang__)

	// These aren't really assembly routines, but we define them so we aren't asm dependent.

	extern "C" void vdasm_uint128_add(uint64 dst[2], const uint64 x[2], const uint64 y[2]) {
		unsigned __int128 a = ((unsigned __int128)x[1] << 64) | x[0];
		unsigned __int128 b = ((unsigned __int128)y[1] << 64) | y[0];
		unsigned __int128 sum = a + b;
		dst[0] = (uint64)sum;
		dst[1] = (uint64)(sum >> 64);
	}

	extern "C" void vdasm_uint128_sub(uint64 dst[2], const uint64 x[2], const uint64 y[2]) {
		unsigned __int128 a = ((unsigned __int128)x[1] << 64) | x[0];
		unsigned __int128 b = ((unsigned __int128)y[1] << 64) | y[0];
		unsigned __int128 diff = a - b;
		dst[0] = (uint64)diff;
		dst[1] = (uint64)(diff >> 64);
	}
